...
const FREE_MODELS = [
  // Groq 系列 - 速度最快，免费额度最高（每天1万4千次）
  {
    ...
    call: (messages: Message[], maxTokens: number) => callGroq(messages, maxTokens, "llama-3.1-8b-instant"),
  },
  {
    name: "groq-llama-70b",
    enabled: () => !!GROQ_API_KEY,
    call: (messages: Message[], maxTokens: number) => callGroq(messages, maxTokens, "llama-3.1-70b-versatile"),
  },
  {
    name: "groq-mixtral",
    enabled: () => !!GROQ_API_KEY,
    call: (messages: Message[], maxTokens: number) => callGroq(messages, maxTokens, "mixtral-8x7b-32768"),
  },
  {
    name: "groq-gemma",
    enabled: () => !!GROQ_API_KEY,
    call: (messages: Message[], maxTokens: number) => callGroq(messages, maxTokens, "gemma2-9b-it"),
  },
  ...
  // OpenRouter 免费模型池 - 模型多，兜底用
  {
    ...
    call: (messages: Message[], maxTokens: number) => callOpenRouter(messages, maxTokens, "meta-llama/llama-3.1-8b-instruct:free"),
  },
  {
    name: "openrouter-llama-3.1-70b",
    enabled: () => !!OPENROUTER_API_KEY,
    call: (messages: Message[], maxTokens: number) => callOpenRouter(messages, maxTokens, "meta-llama/llama-3.1-70b-instruct:free"),
  },
  {
    name: "openrouter-gemma-2-9b",
    enabled: () => !!OPENROUTER_API_KEY,
    call: (messages: Message[], maxTokens: number) => callOpenRouter(messages, maxTokens, "google/gemma-2-9b-it:free"),
  },
  {
    name: "openrouter-qwen-2.5-7b",
    enabled: () => !!OPENROUTER_API_KEY,
    call: (messages: Message[], maxTokens: number) => callOpenRouter(messages, maxTokens, "qwen/qwen-2.5-7b-instruct:free"),
  },
  {
    name: "openrouter-deepseek-r1",
    enabled: () => !!OPENROUTER_API_KEY,
    call: (messages: Message[], maxTokens: number) => callOpenRouter(messages, maxTokens, "deepseek/deepseek-r1-distill-llama-70b:free"),
  },
  {
    name: "openrouter-mistral-7b",
    enabled: () => !!OPENROUTER_API_KEY,
    call: (messages: Message[], maxTokens: number) => callOpenRouter(messages, maxTokens, "mistralai/mistral-7b-instruct:free"),
  },
  {
    name: "openrouter-phi-3",
    enabled: () => !!OPENROUTER_API_KEY,
    call: (messages: Message[], maxTokens: number) => callOpenRouter(messages, maxTokens, "microsoft/phi-3-medium-128k-instruct:free"),
  },
];
...
function withTimeout<T>(promise: Promise<T>, ms: number = 6000, name: string): Promise<T> {
  return Promise.race([
    promise,
...
// ---------- 调用 AI（自动切换所有免费模型） ----------
export async function callAI(messages: Message[], maxTokens: number = 1024): Promise<string> {
  // 精简风格指令，减少token，速度更快
...
// ---------- Google Gemini API ----------
async function callGemini(messages: Message[], maxTokens: number): Promise<string> {
  try {
...
// ---------- OpenRouter API ----------
async function callOpenRouter(messages: Message[], maxTokens: number, model: string): Promise<string> {
  try {
...
// ---------- Groq API ----------
async function callGroq(messages: Message[], maxTokens: number, model: string): Promise<string> {
  try {
